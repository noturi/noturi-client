...
export class MemoService {
   ...
   * 메모 삭제 전 확인
   */
  async deleteMemoWithConfirmation(id: number, skipConfirmation: boolean = false): Promise<void> {
    if (!skipConfirmation) {
  ...
  async getMemoInsights() {
    ...
    const insights = {
      ...
      topCategory: categories.reduce((prev, current) => 
  ...
  async exportMemos(format: 'json' | 'csv' | 'txt' = 'json') {
    ...
    switch (format) {
      case 'json':
        return JSON.stringify(allMemos.memos, null, 2);
      case 'csv':
        // CSV 변환 로직 (향후 구현)
